...
namespace PICkit2V2
{
    public class PICkitFunctions
    {
        ...
        public static void WriteConfigOutsideProgMem(bool codeProtect, bool dataProtect)
        {
            int configWords = DevFile.PartsList[ActivePart].ConfigWords;
            ...
            {
                uint configWord = DeviceBuffers.ConfigWords[i];
        ...
        public static KONST.PICkit2USB DetectPICkit2Device()
        {
            bool result = USB.Find_This_Device(KONST.MChipVendorID, KONST.Pk2DeviceID,
                                 0, ref usbReadHandle, ref usbWriteHandle);
            if (result)
            {
        ...
        public static bool ReadDeviceFile()
        {
            string DeviceFileName = "PK2DeviceFile.dat";
                ...
                try
                {
                    ...
                    using (BinaryReader binRead = new BinaryReader(fsDevFile))
                    {
                        ...
                        for (int l_x = 0; l_x < DevFile.Info.NumberParts; l_x++)
                        {
                            ...
                            for (int l_index = 0; l_index < KONST.NumConfigMasks; l_index++)
                            {
                            ...
                            DevFile.PartsList[l_x].UNUSED3 = binRead.ReadUInt32();
        ...
        public static int DataDownload(byte[] dataArray, int startIndex)
        // returns index of next byte to be transmitted. 0 = failed
        {
            if (startIndex >= dataArray.Length)
            {
        ...
        protected static bool writeUSB(byte[] commandList)
        {
            int bytesWritten = 0;
        ...
        private static bool searchDevice(int familyIndex, bool resetOnNoDevice, bool keepVddOn)
        {
            ...
            uint deviceID = (uint)(Usb_read_array[3] * 256 + Usb_read_array[2]);
            ...
            if ((ActivePart == lastFoundPart) && (scriptBufferChecksum == getScriptBufferChecksum()))
